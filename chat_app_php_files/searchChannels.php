...
} elseif (isset($data['currentUser']) && !empty(trim($data['currentUser']))) {
    // Only user1 is filled, run a query to search all rows where user1 matches
    $currentUser = trim($data['currentUser']);
    $sql = "SELECT * FROM channels WHERE user1 = '{$currentUser}' OR user2 = '{$currentUser}'";
